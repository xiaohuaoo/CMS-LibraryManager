            ...
            <table class="layui-table" lay-data="{
                ...
                <tbody>
                    <?php
                        ...
                        if ($book_name == '') {
                            alert('搜索的内容不能为空', 'borrow_list.php');
                        }
                        $sql = "select * from borrow_list where book_name like '%".$book_name."%' order by id";
                        $rs = mysqli_query($conn, $sql);
                        if ($row = mysqli_num_rows($rs) < 1) {
                            alert('未找到检索的内容', 'borrow_list.php');
                        }
                        if ($rs) {
                            if ($rows['state'] == 0) {
                                $state = '未处理';
            ...
            <script>
                layui.use('table', function () {
                    ...
                    table.on('tool(test)', function (obj) {
                        ...
                        if (eventName == 'del') {
                            // 删除
                            layer.confirm('您确定从书库里删除这本书吗？', function(index) {
                                obj.del();
                                layer.close(index);
